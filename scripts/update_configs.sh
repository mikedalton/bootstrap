...
# Script: update_configs.sh
# Iterates through module directories and copies configuration files
# from the system into the corresponding files in the repo module
...
for module in "${MODULES[@]}"; do
    ...
    if [[ -n "$config_block" ]]; then
        ...
        cmd="CONFIG_FILES=$content; for v in \"\
\${CONFIG_FILES[@]}\"; do printf '%s\\n' \"\$v\"; done"
        # Execute in a clean bash to expand variables like $HOME
        mapfile -t CONFIG_PATHS < <(bash -c "$cmd")
    else
